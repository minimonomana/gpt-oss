...
def collect_effective_keys_from_index(
        index_path: str, base_dir: str) -> Dict[str, TensorProvider]:
    """
    ...
    for key in keys:
        if key.endswith(".blocks"):
            ...
            if scales in keys:
                ...
                def make_loader(blocks_key=key,
                                scales_key=scales,
                                blocks_path=blk_file,
                                scales_path=scl_file):

                    def _ld():
                        with safe_open(blocks_path,
                                       framework="pt",
                                       device="cpu") as fb:
                            blk = fb.get_tensor(blocks_key)
                        with safe_open(scales_path,
                                       framework="pt",
                                       device="cpu") as fs:
...
def write_weights_streaming(
    ...
    """
    np_dtype = np.float32 if out_dtype == "float32" else np.uint16  # bf16 as raw payload
